...
一个**模型 (Model)** $\mathcal{M}$ 是一个五元组 $\mathcal{M} = \langle S, V, E, R, J \rangle$，其构成如下：
...
*   **J:** 一个断言函数 (an assertion function) $J: S \to \mathcal{P}(\text{WFFs})$。对每个状态 $s \in S$，$J_s \subseteq \text{WFFs}$ 是一个在该状态下被**断言 (asserted)** 的公式集合。

#### **5.2. $\mathcal{L_Q}$-模型的约束条件 (Constraints for $\mathcal{L_Q}$-Models)**

一个模型 $\mathcal{M}$ 是一个 **$\mathcal{L_Q}$-模型**，当且仅当它的 $E$ 和 $R$ 组件对任意状态 $s \in S$ 和任意合式公式 $\phi, \psi, \chi \in \text{WFFs}$ 满足以下所有约束条件：

*   **C_A1:** $Q(\phi) \in E_s(\phi)$。
*   **C_A2:** $(Q(\phi) \triangleleft \psi) \preceq_s (\phi \rightsquigarrow \psi)$。
*   **C_A3:** $\phi \preceq_s \phi$。
*   **C_R1:** 若存在 $\xi \in \text{WFFs}$ 使得 $\xi \in E_s(\phi)$ 且 $\xi \in E_s(\chi)$，则 $\phi \preceq_s \chi$
...
注：约束 C_A3 和 C_R3 共同要求 $\preceq_s$ 是一个**预序关系 (preorder)**。断言函数 $J$ 不受任何约束。

#### **5.3. 满足关系 (Satisfaction Relation)**

对于一个 $\mathcal{L_Q}$-模型 $\mathcal{M} = \langle S, V, E, R, J \rangle$，一个状态 $s \in S$ 和一个合式公式 $\phi$，我们归纳定义**满足关系** $\mathcal{M}, s \models \phi$（读作“$\phi$ 在模型 $\mathcal{M}$ 的状态 $s$ 中为真”）如下：

*   **原子命题:** $\mathcal{M}, s \models p \iff s \in V(p)$，对于 $p \in \mathbb{P}$。
*   **Q 算子:** $\mathcal{M}, s \models Q(\phi) \iff Q(\phi) \in J_s$。
...
**证明:** 设 $\mathcal{M} = \langle S, V, E, R, J \rangle$ 为任意 $\mathcal{L_Q}$-模型， $s \in S$ 为任意状态。

1.  **证明 $\models Q(\phi) \triangleleft \phi$ (A1):**
    *   根据满足关系定义 (5.3)，$\mathcal{M}, s \models Q(\phi) \triangleleft \phi$ 成立，当且仅当 $Q(\phi) \in E_s(\phi)$。
    *   根据模型约束 **C_A1**，此条件对任意合法的 $\mathcal{L_Q}$-模型恒成立。
...
2.  **证明 $\models (Q(\phi) \triangleleft \psi) \rightsquigarrow (\phi \rightsquigarrow \psi)$ (A2):**
    *   根据满足关系定义 (5.3)，$\mathcal{M}, s \models (Q(\phi) \triangleleft \psi) \rightsquigarrow (\phi \rightsquigarrow \psi)$ 成立，当且仅当 $(Q(\phi) \triangleleft \psi) \preceq_s (\phi \rightsquigarrow \psi)$。
    *   根据模型约束 **C_A2**，此条件对任意合法的 $\mathcal{L_Q}$-模型恒成立。
...
3.  **证明 $\models \phi \rightsquigarrow \phi$ (A3):**
    *   根据满足关系定义 (5.3)，$\mathcal{M}, s \models \phi \rightsquigarrow \phi$ 成立，当且仅当 $\phi \preceq_s \phi$。
    *   根据模型约束 **C_A3**，此条件对任意合法的 $\mathcal{L_Q}$-模型恒成立。
    *   由于 $\mathcal{M}$ 和 $s$ 是任意的，因此 $\models \phi \rightsquigarrow \phi$。

#### **6.2. 引理：推理规则的保真性**

所有推理规则 R1-R5 都保持真理。即，对于每个规则，若其所有前提在任意模型 $\mathcal{M}$ 的任意状态 $s$ 中为真，则其结论在该模型和状态中也为真。

**证明:** 设 $\mathcal{M} = \langle S, V, E, R, J \rangle$ 和 $s \in S$ 是任意的。
...
4.  **断言函数:** $J_{s_0} = \emptyset$。
5.  **转化与关系函数的构造:**
    令 $E_{s_0}$ 和 $R_{s_0}$ 分别是通过以下归纳构造生成的最小集合：
    *   **基础集 (Stage 0):**
    ...
    *   **归纳步骤 (Stage n+1):**
        *   $E_{s_0}^{(n+1)}(\chi) := E_{s_0}^{(n)}(\chi) \cup \{Q(\phi) \mid \exists \psi, (Q(\phi) \in E_{s_0}^{(n)}(\psi) \land (\psi, \chi) \in R_{s_0}^{(n)})\} \cup \{Q(\phi) \mid \exists \psi, (Q(\phi) \in E_{s_0}^{(n)}(\psi) \land Q(\psi) \in E_{s_0}^{(n)}(\chi))\}$。
...
**断言:** $\mathcal{M}_c = \langle \{s_0\}, V, E_{s_0}, R_{s_0}, J_{s_0} \rangle$ 是一个合法的 $\mathcal{L_Q}$-模型。

**证明:**
为证明此断言，我们必须验证构造出的 $E_{s_0}$ 和 $R_{s_0}$ 满足所有模型约束 C_A1 至 C_R5。$V$ 和 $J$ 的定义是合法的，因其不受任何约束。

*   **C_A1: $Q(\phi) \in E_{s_0}(\phi)$**
    *   根据 Stage 0 的定义，$Q(\phi) \in E_{s_0}^{(0)}(\phi)$，因此 $Q(\phi) \in E_{s_0}(\phi)$。

*   **C_A2: $(Q(\phi) \triangleleft \psi) \preceq_{s_0} (\phi \rightsquigarrow \psi)$**
    *   根据 Stage 0 的定义，$((Q(\phi) \triangleleft \psi), (\phi \rightsquigarrow \psi)) \in R_{s_0}^{(0)}$，因此它在 $R_{s_0}$ 中。

*   **C_A3: $\phi \preceq_{s_0} \phi$**
    *   根据 Stage 0 的定义，$(\phi, \phi) \in R_{s_0}^{(0)}$，因此它在 $R_{s_0}$ 中。

*   **C_R1: 若存在 $\xi$ 使得 $\xi \in E_{s_0}(\phi)$ 且 $\xi \in E_{s_0}(\psi)$，则 $\phi \preceq_{s_0} \psi$。**
    *   假设 $\xi \in E_{s_0}(\phi)$ 且 $\xi \in E_{s_0}(\psi)$。存在 $k$ 使得 $\xi \in E_{s_0}^{(k)}(\phi)$ 且 $\xi \in E_{s_0}^{(k)}(\psi)$。根据 Stage k+1 的构造规则，$(\phi, \psi) \in R_{s_0}^{(k+1)}$，因此 $(\phi, \psi) \in R_{s_0}$。

*   **C_R2: 若 $\phi \in E_{s_0}(\psi)$, 则 $Q(\phi) \preceq_{s_0} \psi$。**
    *   假设 $\phi \in E_{s_0}(\psi)$。存在 $k$ 使得 $\phi \in E_{s_0}^{(k)}(\psi)$。根据 Stage k+1 的构造规则，$(Q(\phi), \psi) \in R_{s_0}^{(k+1)}$，因此 $(Q(\phi), \psi) \in R_{s_0}$。

*   **C_R3: 若 $\phi \preceq_{s_0} \psi$ 且 $\psi \preceq_{s_0} \chi$, 则 $\phi \preceq_{s_0} \chi$。**
    *   假设 $(\phi, \psi) \in R_{s_0}$ 且 $(\psi, \chi) \in R_{s_0}$。存在 $k$ 使得 $(\phi, \psi) \in R_{s_0}^{(k)}$ 且 $(\psi, \chi) \in R_{s_0}^{(k)}$。根据 Stage k+1 的构造规则，$(\phi, \chi) \in R_{s_0}^{(k+1)}$，因此 $(\phi, \chi) \in R_{s_0}$。

*   **C_R4: 若 $Q(\phi) \in E_{s_0}(\psi)$ 且 $\psi \preceq_{s_0} \chi$, 则 $Q(\phi) \in E_{s_0}(\chi)$。**
    *   假设 $Q(\phi) \in E_{s_0}(\psi)$ 且 $(\psi, \chi) \in R_{s_0}$。存在 $k$ 使得 $Q(\phi) \in E_{s_0}^{(k)}(\psi)$ 且 $(\psi, \chi) \in R_{s_0}^{(k)}$。根据 Stage k+1 的构造规则，$Q(\phi) \in E_{s_0}^{(k+1)}(\chi)$，因此 $Q(\phi) \in E_{s_0}(\chi)$。

*   **C_R5: 若 $Q(\phi) \in E_{s_0}(\psi)$ 且 $Q(\psi) \in E_{s_0}(\chi)$, 则 $Q(\phi) \in E_{s_0}(\chi)$。**
    *   假设 $Q(\phi) \in E_{s_0}(\psi)$ 且 $Q(\psi) \in E_{s_0}(\chi)$。存在 $k$ 使得 $Q(\phi) \in E_{s_0}^{(k)}(\psi)$ 且 $Q(\psi) \in E_{s_0}^{(k)}(\chi)$。根据 Stage k+1 的构造规则，$Q(\phi) \in E_{s_0}^{(k+1)}(\chi)$，因此 $Q(\phi) \in E_{s_0}(\chi)$。
...
*   $\mathcal{M}_c, s_0 \not\models q$，因为 $s_0 \notin V(q)$。这证明了语义是**非平凡的**。
*   对于任意公式 $\phi$, $\mathcal{M}_c, s_0 \not\models Q(\phi)$，因为 $Q(\phi) \notin J_{s_0}$ (由于 $J_{s_0} = \emptyset$)。这证明了 `Q` 算子在语义上是**非平凡的**，因为存在一个模型使其为假。
