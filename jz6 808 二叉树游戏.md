...
*   **节点集合** $V_{\Omega} = \{0, 1\}^*$（由0和1构成的有限字符串集合）。
    *   $\lambda$ 表示初始全局根节点（空串）。
    *   对于任意 $u \in V_{\Omega}$，定义其**固有左子节点**为 $u0$，**固有右子节点**为 $u1$；并定义全局前驱函数 $\pi : V_{\Omega} \setminus \{\lambda\} \to V_{\Omega}$，使得对于任意形如 $u\sigma$ 的节点（其中 $\sigma \in \{0, 1\}$），其**固有父节点**为 $\pi(u\sigma) = u$。
*   **基础边集** $E_{\Omega}$。
...
1.  **当前图结构** $G_t = (V_{\Omega}, E_t)$：
    *   当前边集 $E_t$ 通过与全空间基础边集 $E_{\Omega}$ 的关系，被严格划分为不相交的两个子集：$E_t = E_{T,t} \cup E_{V,t}$。
    *   **当前树边集** $E_{T,t} = E_t \cap E_{\Omega}$。对于任意 $e \in E_{T,t}$，其固有类型保持为 $\tau(e) = \mathbf{T}$ (Tree Edge)。
    *   **当前虚边集** $E_{V,t} = E_t \setminus E_{\Omega}$。对于任意 $e \in E_{V,t}$，其类型定义为 $\tau(e) = \mathbf{V}$ (Virtual Edge)。
...
### 定义 2.1 (路径属性)
在配置 $G_t$ 中，路径 $P$ 定义为**简单**顶点序列 $(v_0, v_1, \dots, v_k)$（即对于任意 $i \neq j$，均有 $v_i \neq v_j$），满足 $\{v_i, v_{i+1}\} \in E_t$。
令 $e_i = \{v_{i-1}, v_i\}$，定义路径的边类型序列为 $\Sigma_P = (\tau(e_1), \dots, \tau(e_k))$。

#### 2.1.1 密度约束 (Density Constraint)
谓词 $\Phi_{\text{dense}}(P)$ 为真，当且仅当序列 $\Sigma_P$ 中**不包含如下模式的连续子串**：两端为 $\mathbf{V}$ 边且中间夹有 $2$ 条或以上的 $\mathbf{T}$ 边。
形式化地：不存在索引 $i < j$，使得 $\tau(e_i) = \mathbf{V}$ 且 $\tau(e_j) = \mathbf{V}$，同时满足 $j - i - 1 \ge 2$ 且对所有 $i < m < j$ 均有 $\tau(e_m) = \mathbf{T}$。
...
玩家在 $V_{\Omega}$ 中选择两个不相等的节点 $u, v$（$u \neq v$），且要求当前两节点尚未直接连接（即 $\{u, v\} \notin E_t$），以此构造候选边 $e_{new} = \{u, v\}$。
系统根据全空间拓扑自动确定其类型约束：若 $e_{new} \in E_{\Omega}$，则 $\tau(e_{new}) = \mathbf{T}$（即曾经崩塌的固有树边被重新连通）；若 $e_{new} \notin E_{\Omega}$，则 $\tau(e_{new}) = \mathbf{V}$。
...
*   **判定条件**：
	...
	2.  **孤儿约束**：被选中的 $v_{orphan}$ 必须满足“无父节点”属性，即在当前边集 $E''_{t}$ 中，不存在与其固有父节点连接的边（对于全局原点 $\lambda$，此条件天然满足）：
	    $$ v_{orphan} = \lambda \quad \lor \quad \{v_{orphan}, \pi(v_{orphan})\} \notin E''_{t} $$*
    3.  **状态更新**：
